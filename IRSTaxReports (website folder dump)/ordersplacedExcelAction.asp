		...
		if AllUserId <> "" Then
			...
			Do While Not rslist.EOF
				...
				if rslist("fldTypeofForm") = "6"  Then
					...
					if rslist("fldLoanNumber") <> "" Then
						flag = ""
						if intcounter = 0 Then
							myLoanNumber(intcounter) = rslist("fldLoanNumber")
							myAmount(intcounter) = rslist("ssn_fee")
							intcounter = intcounter + 1
						else
							''For j = 0 to intCounter-1
							''	if rslist("fldLoanNumber") = myLoanNumber(j) Then
							''		Response.Write rslist("fldLoanNumber") & "IF" & "<BR>"
							''		myAmount(j) = myAmount(j) + rslist("ssn_fee")
							''	else
							''		Response.Write rslist("fldLoanNumber") & "ELSE" & "<BR>"
							''		Redim Preserve myLoanNumber(intCounter)
							''		Redim Preserve myAmount(intCounter)
							''		myLoanNumber(j) = rslist("fldLoanNumber")
							''		myAmount(j) = rslist("ssn_fee")
							''		intcounter = intcounter + 1				
							''	End if
							''Next
							...
							Next
							if flag <> "" Then
								myAmount(flag) = myAmount(flag) + rslist("ssn_fee")																		
							else
								Redim Preserve myLoanNumber(intCounter)
								Redim Preserve myAmount(intCounter)
								myLoanNumber(intCounter) = rslist("fldLoanNumber")
					...
					if rslist("fldLoanNumber") <> "" or rslist("fldLoanNumber") <> NULL Then
						flag = ""
						if intcounter = 0 Then
							myLoanNumber(intcounter) = rslist("fldLoanNumber")
							myAmount(intcounter) = irsfee
							intcounter = intcounter + 1
						else
							For j = 0 to intCounter-1
								if rslist("fldLoanNumber") = myLoanNumber(j) Then
							...
							if flag <> "" Then
								myAmount(flag) = myAmount(flag) + irsfee																		
							else
								Redim Preserve myLoanNumber(intCounter)
								Redim Preserve myAmount(intCounter)
								myLoanNumber(intCounter) = rslist("fldLoanNumber")
				...
				rslist.MoveNext
			Loop
			ExportFile.writeline "<tr><td align='center' colspan='10' align='left'>&nbsp;</td></tr>"
			For intC = 0 to intcounter-1
				ExportFile.writeline "<tr><td align='center' colspan='10' align='left' bgcolor='#C0C0C0'><b>Total For Loan Number "& myLoanNumber(intC) & " :<B> $ " & myAmount(intC) &"</td></tr>"
			Next 
		End If
