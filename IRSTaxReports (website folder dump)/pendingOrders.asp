...
<%
	...
	if not IsEmpty(Request.Form("btnSubmit")) then
		...
		Todate = m_dtmTo
		Dim cSql
		cSql = "select fldrequestname,fldssnno,fldTypeofform,fldTaxyear2005,fldTaxyear2006,fldTaxyear2007,fldTaxyear2008,fldtaxyear2004,fldordernumber From tblorder where fldOrderDate <= '"& Todate &"' and fldOrderDate > '12/31/2009' AND fldStatus = 'p' AND fldTypeofform in (1,2,3) ORDER BY fldOrderdate desc"
'		Response.Write cSql
		...
		if not rsUid.EOF then
			...
			while not rsUid.EOF 
				...
				str = i+1 & "." & " " & Trim(rsUid("fldrequestname")) & " " & Trim(rsUid("fldssnno")) & " " & year1
				filetxt.WriteLine(str)				
					sqlcomments = "select * from tblComments where OrderId="& rsUid("fldordernumber") & " order by fldCommentDate desc"
					set rscomments = conn.execute (sqlcomments)	
					while not rscomments.EOF
						filetxt.WriteLine(rscomments("fldComments"))
						rscomments.Movenext()
					wend
				i=i+1
				rsUid.movenext
			wend
		End if
		cSql = "select fldrequestname,fldssnno,fldTypeofform,fldTaxyear2005,fldTaxyear2006,fldTaxyear2007,fldTaxyear2008,fldtaxyear2004,fldordernumber From tblorder where fldOrderDate <= '"& Todate &"' and fldOrderDate > '12/31/2009' AND fldStatus = 'p' AND fldTypeofform in (4,5) ORDER BY fldOrderdate desc"
		set rsUid = Server.CreateObject("Adodb.Recordset")
		rsUid.open cSql,conn,3,2
		if not rsUid.EOF then
			filetxt.WriteLine("")
